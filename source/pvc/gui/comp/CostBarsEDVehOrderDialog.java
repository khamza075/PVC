...
public class CostBarsEDVehOrderDialog extends JDialog implements ActionListener {
	//Sizing constants
	...
	public CostBarsEDVehOrderDialog(CostBarsDisplaySetup displaySetup) {
		//Super
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (source == btnUp) {
			...
			dSetup.vehPlotID_moveUp(curSel);
			rePopulateList();
		...
		if (source == btnDown) {
			...
			dSetup.vehPlotID_moveDown(curSel);
			rePopulateList();
	...
	private void rePopulateList() {
		...
		for (int i=0; i<st.length; i++) st[i] = new String(dSetup.vehModelShortName(i));
