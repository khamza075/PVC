...
public class GHGHistVehOrderDialog extends JDialog implements ActionListener {
	//Sizing constants
	...
	public GHGDisplaySetup getDisplaySetup() {return new GHGDisplaySetup(dSetup);}

	
	public GHGHistVehOrderDialog(AnalysisVehModelsSetup avModelsSetup, GHGDisplaySetup displaySetup) {
		//Super
		...
		vms = avModelsSetup.vehModelsSetup();
	...
	private void updateButtonsActivity() {
		...
		if (curSel < (dSetup.numVehicles()-1)) btnDown.setEnabled(true);
		else btnDown.setEnabled(false);
	}
	private void rePopulateList() {
		String[] st = new String[dSetup.numVehicles()];
		for (int i=0; i<st.length; i++) st[i] = new String(vms[dSetup.getVehAtDisplayPos(i).vehID()].shortName);
